...
**Reference:** `lib/AGENTS.md`, `lib/manifests/`, `lib/core/routing/router.dart` (AppRoute is defined there; there is no separate app_route file).
...
- **Scope B – Feature:** Entire feature directory under `lib/presentation/{feature}/`, plus:
  - Route(s) in `lib/core/routing/router.dart` (AppRoute constant and GoRoute)
...
3. **Read routing:** `lib/core/routing/router.dart` (AppRoute class and GoRoute definitions).
4. **Discover all references** (use grep/search; do not guess):
   - **Imports:** Any file that imports the target file or a removed class.
   - **Route usage:** `AppRoute.<routeName>`, `context.go(AppRoute.xxx)`, `context.push(AppRoute.xxx)`. Search in `lib/` (and optionally `test/`).
...
1. **Is the target a route/screen?** (View is registered in `router.dart` with AppRoute and GoRoute.) → **WARNING:** Yes.
...
1. **References to remove first:** Entry points → route definition and AppRoute constant in `lib/core/routing/router.dart` → imports and usages in other files → provider registration.
...
1. Apply reference removals in order: entry points → route definition and AppRoute constant → imports and usages → provider registration.
...
- **Never remove** a AppRoute constant or GoRoute without removing/updating every place that navigates to that route.
